...
@WebServlet(name = "UsuarioServlet", urlPatterns = {"/UsuarioServlet", "/Login"})
        ...
        try {
            ...
            if ("Registrar".equals(accion)) {
                ...
                if (dao.correoExiste(correo)) {
                    ...
                    String codigo = Seguridad.generarCodigo();

                    boolean ok = dao.registrar(usuario, codigo);
                    ...
                    try {
                        EmailUtil.enviarCodigoVerificacion(correo, usuario.getNombre(), codigo);
                    } catch (Exception ex) {
                        // La cuenta SI quedo creada, solo fallo el envio del correo.
                        request.setAttribute("error", "Tu cuenta se creo, pero no se pudo enviar el correo (" + ex.getMessage() + "). Intenta reenviar el codigo.");
                    }

                    response.sendRedirect("VerificarCuenta.jsp?correo=" + java.net.URLEncoder.encode(correo, "UTF-8"));
                    return;
                ...
                boolean ok = dao.verificarCodigo(correo, codigo);
                ...
                if (ok) {
                    ...
                    request.setAttribute("error", "Codigo incorrecto o ya usado");
                    request.setAttribute("correo", correo);

                    RequestDispatcher rd = getServletContext().getRequestDispatcher("/VerificarCuenta.jsp");
                    rd.forward(request, response);
                    return;
                ...
                boolean ok = dao.actualizarCodigo(correo, codigo);

                if (ok) {

                    try {
                        EmailUtil.enviarCodigoVerificacion(correo, "", codigo);
                        request.setAttribute("mensaje", "Te enviamos un nuevo codigo a tu correo");
                    } catch (Exception ex) {
                        request.setAttribute("error", "No se pudo enviar el correo: " + ex.getMessage());
                    }

                } else {

                    request.setAttribute("error", "Ese correo no existe o ya esta verificado");
                ...
                } else if (u != null && !u.isVerificado()) {

                    request.setAttribute("error", "Tu cuenta todavia no esta verificada. Revisa el codigo que te enviamos por correo.");
                    request.setAttribute("correo", correo);
